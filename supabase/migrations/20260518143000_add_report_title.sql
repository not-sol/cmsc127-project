...
set
  title = nullif(split_part(remarks, E'\n\n', 1), ''),
  remarks = nullif(substr(remarks, length(split_part(remarks, E'\n\n', 1)) + 3), '')
where title is null
  and remarks like '%' || E'\n\n' || '%';

drop function if exists public.create_draft_accomplishment_report(date, date, text, bigint);
...
revoke execute on function public.create_draft_accomplishment_report(date, date, text, bigint, text) from public, anon;
grant execute on function public.create_draft_accomplishment_report(date, date, text, bigint, text) to authenticated;
