...
export type ReportEntryDetailGroup = {
  label: string
  values: Record<string, ReportEntryDetailValue>
}
...
export async function fetchReportEntryDetails(entryId: number): Promise<ReportEntryDetailGroup[]> {
  const mergedValues: Record<string, ReportEntryDetailValue> = {}
  ...
  if (!baseError && baseRow) {
    Object.assign(mergedValues, await cleanValues(baseRow as Record<string, unknown>))
  }
